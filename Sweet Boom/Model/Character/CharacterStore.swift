...
class CharacterStore: ObservableObject {
    ...
    func getButtonText(for character: Character) -> String {
        if character.isOwned && character.isChoosen {
            ...
            let buyButtonImageName = getPriceImageButton(for: character)
            if character.price > coinsManager.currentCoins {
                return buyButtonImageName+"NotEnough"
            } else {
                return buyButtonImageName
            }
        }
    }
    
    private func getPriceImageButton(for character: Character) -> String {
        switch character.price {
        case 50:
            return "buy50"
        case 100:
            return "buy100"
